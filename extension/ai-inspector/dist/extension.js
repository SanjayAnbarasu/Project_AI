...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode2.commands.registerCommand("aiInspector.forgetFile", () => {
      const activeEditor = vscode2.window.activeTextEditor;
      if (!activeEditor) {
        vscode2.window.showWarningMessage("Ai-Inspector: No active file to forget.");
        return;
      }
      ...
      vscode2.window.showInformationMessage(`Ai-Inspector: Cleared retry/fix history for ${filePath.split(/[\\/]/).pop()}.`);
    })
  );
  context.subscriptions.push(
...
async function getSourceContext(filePath, lineNumber, windowSize = 40) {
  try {
    ...
    if (document.lineCount <= WHOLE_FILE_LINE_LIMIT) {
      const zeroIndexed2 = Math.max(0, Math.min(lineNumber - 1, document.lineCount - 1));
      const lines2 = [];
      for (let i = 0; i < document.lineCount; i++) {
        const marker = i === zeroIndexed2 ? ">>" : "  ";
        lines2.push(`${marker} ${i + 1}: ${document.lineAt(i).text}`);
      }
      return lines2.join("\n");
    }
...
async function sendLogToBackend(scrubbedText, locationPath, lineFallback, deliveryTag) {
  try {
    ...
    if (responseData.ai_suggestion) {
      ...
      vscode2.window.showWarningMessage("Ai-Inspector: AI backend could not generate a fix this attempt (both engines failed or timed out). Will retry on next crash detection.");
    }
  } catch (error) {
...
async function applyFixToFile(suggestionText, lineNumber, targetFilePath) {
  ...
  try {
    ...
    if (langId === "python") {
      ...
      const identUsedInFix = new Set(cleanSuggestion.match(/\b[a-zA-Z_][a-zA-Z0-9_]*\b/g) || []);
      const funcRanges = [];
      ...
      for (const fn of funcRanges) {
        ...
        if (!isInsideThisFn) {
          console.warn(
            `Ai-Inspector: Scope mismatch -- fix uses parameters of "${fn.name}" (${[...fn.params].join(", ")}) but target line ${lineNumber} is outside that function's body (lines ${fn.start + 1}-${fn.end + 1}). Rejecting.`
          );
          updateInspectorStatus(8 /* ManualMode */);
    ...
    const alreadyTriedVariant = fixHistory.filter((r) => r.filePath === targetFilePath).slice(0, currentEpisodeAttempts).some((r) => {
      try {
    ...
    if (alreadyApplied || alreadyTriedVariant) {
      updateInspectorStatus(8 /* ManualMode */);
      vscode2.window.showWarningMessage("Ai-Inspector: Suggested fix already present or already tried this retry cycle \u2014 skipping duplicate patch.");
      return false;
    }
